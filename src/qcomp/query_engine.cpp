...
query_engine::query_engine(PContext &ctx, unsigned int thread_num, unsigned cv_range) : thread_num_(thread_num), ctx_(ctx), scheduler_(thread_num), compiled_(false), complete_(false) {
...
void query_engine::generate(JitFromScratch &jit, std::shared_ptr<base_op> query, bool parallel) {
    parallel_ = parallel;
    cur_query_ = query;
    compile_th = std::thread(compile_task(*this, ctx_, jit, query));
    if(!parallel) {
        compile_th.join();
    }
...
void query_engine::prepare(JitFromScratch &jit, graph_db_ptr graph) {
...
void query_engine::run(JitFromScratch &jit, graph_db_ptr graph, result_set * rs, std::vector<uint64_t*> args) {
    auto tx = graph->begin_transaction();
    current_transaction_ = tx;
    if(parallel_) {
        unsigned int op_start = 1;
        arg_builder args;
        interprete_visitor iv(graph, args, rs);
    ...
    for(i = start_idx; i >= 0; i--) {
        start_[i](graph.get(), 0, graph->get_nodes()->num_chunks(), tx, 1, &type_vec_[start_idx], rs, nullptr, finish_[0], 0, args.data());
        offset += offsets[i];
    }
...
void query_engine::run_parallel(JitFromScratch &jit, graph_db_ptr graph, result_set * rs, arg_builder & args, unsigned thread_num) {
    if(!has_join(cur_query_)) {
        ...
        auto chunksz = graph->get_nodes()->num_chunks() / thread_num;
        auto max_chunksz = graph->get_nodes()->num_chunks();
        compile_th.join();
	prepare(jit, graph);
	type_vec_[0].push_back(7);
        ...
        for(auto i = 0u; i < thread_num; i++) {
            ...
            query_threads.push_back(std::thread([&, cur_start, end]{
                current_transaction_ = tx;
                start_[0](graph.get(), cur_start, end, current_transaction_, 1, &type_vec_[0], rs, nullptr, finish_[0], 0, args.args.data());
            }));
            cur_start += chunksz;
        }
...
qscan_task::qscan_task(unsigned tid, graph_db_ptr graph, task_fct fct) : tid_(tid), graph_(graph), task_(fct) {

}

void qscan_task::operator()() {
    task_(graph_, tid_);
    //TODO: finish fct
}
...
task_scheduler::task_scheduler(unsigned int num_threads) : thread_num_(num_threads), running_(false), started_(false){

}

void task_scheduler::operator()() {
