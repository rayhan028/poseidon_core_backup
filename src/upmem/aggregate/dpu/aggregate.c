...
int partition() {
    ...
    prop_code_t* mr_elems = (prop_code_t*) DPU_MRAM_HEAP_POINTER;
    prop_code_t* wr_elems = (prop_code_t*) mem_alloc(NR_WR_ELEMS_PER_TASKLET_PARTITION * sizeof(prop_code_t));

    /* read grouping keys from MRAM and compute histogram */
    for (uint32_t i = tasklet_id * NR_WR_ELEMS_PER_TASKLET_PARTITION; i < dpu_parameters.num_elems; i += NR_WR_ELEMS_PER_TASKLET_PARTITION * NR_TASKLETS) {
        mram_read((__mram_ptr void const*) &mr_elems[i], wr_elems, NR_WR_ELEMS_PER_TASKLET_PARTITION * sizeof(prop_code_t));
        ...
        for (uint32_t j = 0; j < num_elems; j++) {
            ...
            dpu_partition_sizes[partition] += 1;
