...
query_set queryc::generate_qex_plan(graph_db_ptr &gdb, const std::string &qstr) {
  ...
  auto qop = ast_to_qex(ast, gdb, sources);
  query_set qset;
...
std::pair<qop_ptr, qop_ptr> queryc::ast_to_qex(ast_op_ptr &ast, graph_db_ptr& gdb, std::vector<qop_ptr>& sources) {
  qop_ptr qop;
