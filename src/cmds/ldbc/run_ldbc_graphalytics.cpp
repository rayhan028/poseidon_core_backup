...
int main(int argc, char **argv) {
 ...
 try {
    ...
    desc.add_options()
        ...
        ("import,i", value<std::string>(&graphalytics_home), "Path to directories containing SNB CSV files")
    ...
    if (vm.count("import"))
      graphalytics_home = vm["import"].as<std::string>();
  ...
  load_snb_data(graph, graphalytics_home);
#endif
  graph->print_stats();
