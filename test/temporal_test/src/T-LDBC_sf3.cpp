...
const std::string QUERY_CSV = "/home/rayhan/poseidon_core/test/temporal_test/benchmark_data_T_LDBC_sf3/poseidon_query_latencies.csv";
const std::string STORAGE_CSV = "/home/rayhan/poseidon_core/test/temporal_test/benchmark_data_T_LDBC_sf3/poseidon_storage_and_write.csv";
...
int main(int argc, char* argv[]) {
    ...
    // UPDATED HELPER: Logical Historical Size (Uses .size() instead of .capacity())
    /*auto get_hist_logical_size = [&]() {
        size_t h_vecs = (storage->node_deltas_.size() * sizeof(node_history_record)) +
                        (storage->rship_deltas_.size() * sizeof(rship_history_record));
        size_t h_idxs = 0;
        for (auto const& [lid, chain] : storage->node_delta_index_) h_idxs += chain.size() * sizeof(offset_t);
        for (auto const& [lid, chain] : storage->rship_delta_index_) h_idxs += chain.size() * sizeof(offset_t);
        
        return h_vecs + h_idxs;
    };*/
