...
const int BENCHMARK_ITERATIONS_PER_STAGE = 10;
const int WARMUP = 10;
...
// LOGICAL MEMORY MEASUREMENT for current and historical data
...
// OPERATORS AND UTILITIES FOR BENCHMARKING
...
// Simple Zipfian generator for sampling logical IDs based on their access frequency
class ZipfGenerator { 
public:
...
void run_bench_stage(std::shared_ptr<graph_db> db, const std::vector<uint64_t>& active_nodes, uint64_t vt, int iteration, int current_ops) {
    ...
    std::vector<uint64_t> d_windows = {1000, 50000, 100000, 150000, 200000}; // Time windows for Q4 sampling
    constexpr uint64_t FIXED_WINDOW = 50000; // Fixed window for Q1/Q4 sampling to ensure existence of versions
    uint64_t start_window = (vt > FIXED_WINDOW) ? (vt - FIXED_WINDOW) : 0;

    // CATEGORIZATION
    std::vector<std::pair<uint64_t, uint64_t>> updated_vertices;
    for (const auto& [lid, count] : version_tracker) {
        if (count > 0) updated_vertices.push_back({lid, count});
    }
    std::sort(updated_vertices.begin(), updated_vertices.end(),
        [](const auto& a, const auto& b) { return a.second > b.second; });
    
    uint64_t total_accesses = 0;
    for (const auto& [lid, count] : updated_vertices) total_accesses += count;
    ...
    std::vector<uint64_t> hot_nodes, warm_nodes, cold_nodes; // categorization based on access frequency percentiles
    ...
    for (const auto& [lid, count] : updated_vertices) {
        ...
        if (!exists) continue;
        if (cdf < 0.3) hot_nodes.push_back(lid);
        else if (cdf < 0.6) warm_nodes.push_back(lid);
        else cold_nodes.push_back(lid);
    }   

    for (uint64_t lid : active_nodes) {
        if (version_tracker[lid] == 0) {
            db->begin_transaction();
            if (db->try_get_node_at_vt(lid, vt).has_value()) cold_nodes.push_back(lid);
            db->commit_transaction();
        }
    }
    ...
    while (samples_q1_q3.size() < SAMPLES_PER_ITER) {
        ...
        db->begin_transaction();
        if (db->try_get_node_at_vt(id, vt).has_value()) samples_q1_q3.push_back(id);
        db->commit_transaction();
    }
    while (samples_q2.size() < SAMPLES_PER_ITER) {
        uint64_t id = active_nodes[query_zipf.next(local_gen) % active_nodes.size()];
        db->begin_transaction();
        if (db->try_get_node_at_vt(id, start_window).has_value()) samples_q2.push_back(id);
        db->commit_transaction();
    }

    // Use the start of the window for sampling to guarantee existence for Q4
    uint64_t target_q4 = (start_window <= 2000) ? 2005 : start_window;
    int attempts = 0;
    while (samples_q4.size() < SAMPLES_PER_ITER && attempts++ < 10000) {
        uint64_t id = active_nodes[query_zipf.next(local_gen) % active_nodes.size()];
        db->begin_transaction();
        if (db->try_get_node_at_vt(id, target_q4).has_value()) samples_q4.push_back(id);
        db->commit_transaction();
    }

    // Q1/Q2/Q3/Q4 EXECUTION
    ...
    double q1 = (q1_actual == 0) ? 0.0 : std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - t1).count() / (1000.0 * q1_actual);
    ...
    double q2 = (q2_actual == 0) ? 0.0 : std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - t3).count() / (1000.0 * q2_actual);
    ...
    for (size_t i = 0; i < samples_q4.size(); ++i) {
        db->begin_transaction(); 
        auto opt_nid = db->try_get_node_at_vt(samples_q4[i], target_q4);
        if (opt_nid.has_value()) {
            query_ctx ctx(db); 
    ...
    // HOT/WARM/COLD for Q3 access vertex types
    ...
    // TIME SLICE WINDOW LOOP FOR Q4 TO SHOW IMPACT OF WINDOW SIZE ON LATENCY
    ...
    for (auto window : d_windows) {
        ...
        for (size_t i = 0; i < SAMPLES_PER_ITER; ++i) {
            uint64_t rs = time_dist(local_gen);
            uint64_t lid = active_nodes[query_zipf.next(local_gen) % active_nodes.size()];
            db->begin_transaction();
            auto opt_nid = db->try_get_node_at_vt(lid, rs);
            if (opt_nid.has_value()) {
                query_ctx ctx(db);
                auto op = std::make_shared<poseidon::qop_time_slice_expand>(rs, rs + window, true, "");
                op->connect(sink, std::bind(&SilentPrinter::process, sink.get(), std::placeholders::_1, std::placeholders::_2), nullptr);
                qr_tuple t; t.push_back(db->get_node_description(opt_nid.value()));
                op->process(ctx, t); valid_samples++;
            }
            db->commit_transaction();
        }
        double lat = (valid_samples == 0) ? 0.0 : std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count() / (1000.0 * valid_samples);
        fig_d << current_ops << "," << window/1000 << "," << lat << "\n";
    }

    double total_mb = get_total_logical_mb(db);
    std::ofstream(CSV_FILE, std::ios_base::app) << iteration << "," << current_ops << "," << q1 << "," << q2 << "," << q3 << "," << q4 <<"\n";
}
...
int main() {
    ...
    file << "Iteration,Ops_Count,Q1_ms,Q2_ms,Q3_ms,Q4_ms\n";
    file.close();
    ...
    //Total Nodes: 19,543 . Total Edges: 247,175
    ...
    // TEMPORAL VISIBILITY REGISTRATION
    // register'initial nodes in the temporal index
    ...
    db_ptr->run_transaction([&](){
        ...
        for (auto lid : raw_lids) {
            if (lid < 0x7FFFFFFF) {
                ...
                if (phys != UNKNOWN) {
                    db_ptr->update_node_create_version(phys, {}, 2000); 
    ...
    for (int target_ops : OPS_STAGES) {
        ...
        for (int i = 0; i < ops_to_add; ++i) {
            ...
            // 80% updates, 10% new nodes + edges, 10% deletes. Updates are skewed towards more frequently updated vertices using Zipfian sampling
            try {
