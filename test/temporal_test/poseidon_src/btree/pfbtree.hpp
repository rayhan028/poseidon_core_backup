...
class BPTree {
 ...
 public:
  ...
  mutable void *rootNode;  //< pointer to the root node (an instance of @c LeafNode or
 ...
 public:
  ...
  /**
 * range scan was hardened to resolve two critical issues:
 * Logical Leak: Added a lower-bound check (key < minKey) because findLeafNode()
 * may land on a leaf containing keys smaller than the starting range.
 * Segmentation Fault: Added a null-pointer check for next_pid. Previously,
 * at the end of a scan, it attempted to load page 0 and dereference it
 * (leaf->ntype), causing a crash.
 */

  void scan(const KeyType &minKey, const KeyType &maxKey, ScanFunc func) const {
    // reload root node in case the page was evicted
    const_cast<BPTree*>(this)->rootNode = load_node(rootPid);
    auto leaf = findLeafNode(minKey);
    while (leaf != nullptr) {
      for (auto i = 0u; i < leaf->numKeys; i++) {
        auto &key = leaf->keys[i];
        //LOWER BOUND FILTER (Prevents duplicate data)
        // findLeafNode jumps to the correct leaf, but that leaf may contain keys SMALLER than minKey. skip manually.
        if (key < minKey) continue;

        // Upper bound check (Correctly exits the entire scan)
        if (key > maxKey) return;

        auto &val = leaf->values[i];
        func(key, val);
      }
      // NULL POINTER CHECK (Prevents Segmentation Fault)
      // Capture the next page ID before loading. If it's 0, we hit the end.
      paged_file::page_id next_pid = leaf->nextLeaf;
      if (next_pid == 0) break; // Exit safely if there are no more leaves

      leaf = reinterpret_cast<LeafNode *>(load_node(next_pid));
      //  ensure the load succeeded and the node is a leaf
      if (leaf == nullptr) break;
      assert(leaf->ntype == 0);
    }
  }

  // older range scan method
  /* void scan(const KeyType &minKey, const KeyType &maxKey, ScanFunc func) const {
    // make sure we reload the root node in case the page was evicted
    rootNode = load_node(rootPid);
    auto leaf = findLeafNode(minKey);
