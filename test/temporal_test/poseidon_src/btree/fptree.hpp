...
#ifndef DBIS_FPBPTree_hpp_
#define DBIS_FPBPTree_hpp_
 ...
 *
 * @param keysRef a reference to the node's keys to find the minimum key in
...
/**
 * A persistent memory implementation of a FPBPTree.
 *
...
class FPBPTree {
 ...
 public:
   ...
   */
  explicit FPBPTree() : depth(0) {
  // FPBPTree() {
    rootNode = newLeafNode();
    leafList = rootNode.leaf;
    depth = 0;
    std::cout << "created new FPBPTree with sizeof(BranchNode) = "
  ...
  /**
   * Recover the FPBPTree by iterating over the LeafList and using the recoveryInsert method.
   */
  void recover() {
    std::cout << "Starting RECOVERY of FPBPTree \n";
    pptr<LeafNode> currentLeaf = leafList;
    if (leafList == nullptr) {
      std::cout << "No data to recover FPBPTree \n";
      return;
    }
    /* counting leafs */
    auto leafs = 0u;
