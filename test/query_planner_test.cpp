...
TEST_CASE("Testing the poseidon query processor", "[query_proc]") {
    ...
    SECTION("NodeScan") {
        ...
        plan.print_plan(os);
        os << std::ends;
        REQUIRE(std::string(os.str()) == "node_scan([Person]) - { in=0 | out=0 | time=0s }\n");
    }
    ...
    SECTION("CrossJoin") {
        ...
        plan.print_plan(os);
        os << std::ends;
        REQUIRE(std::string(os.str()) == "limit([20]) - { in=0 | out=0 | time=0s }\n└── cross_join() - { in=0 | out=0 | time=0s }\n    ├── node_scan([Comment]) - { in=0 | out=0 | time=0s }\n    └── node_scan([Person]) - { in=0 | out=0 | time=0s }\n");
    }
