...
const std::string test_path = poseidon::gPmemPath + "qcompiler_is_test";
const std::string test_path2 = poseidon::gPmemPath + "qcompiler_iu_test";
...
TEST_CASE("Testing LDBC IS queries in interpreted mode", "[qinterp]") {
    ...
    SECTION("IS #1") {
      spdlog::info("LDBC IS#1"); 
        ...
        auto res = qp.execute_query(qproc::Compile, qstr);
